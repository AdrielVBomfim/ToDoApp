...
class AddToDoPageState extends State<AddToDoPage> {
  ...
  void _handleAddToDo(AsyncSnapshot<List<ToDo>> snapshot) {
    setState(() {
      ...
      ToDo temp = ToDo(
          ...
          dueDate: ifDueDate ? newTime.toString() : null);

      if (snapshot.data.length == 0)
        _todosBloc.inAddTodo.add(temp);
      else
        _todosBloc.inDeleteTodo.add(temp.id);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                      ...
                      child: GestureDetector(
                        ...
                        onTapUp: (TapUpDetails details) {
                          ...
                          Future.delayed(const Duration(seconds: 3))
                              .then((_) => _handleAddToDo(snapshot));
                        },
      ...
      drawer:
          SideBar(), // This trailing comma makes auto-formatting nicer for build methods.
    );
  }
}
